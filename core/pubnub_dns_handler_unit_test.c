...
static const char encoded_piece21[] = 
    "\77[<-------the_Longest_Label_Stretch-------63_characters------->]\300\277";
...
AfterEach(pubnub_dns_handler)
{
    PUBNUB_LOG_TRACE("========================================================\n");
}


Ensure(pubnub_dns_handler, decodes_well_strange_response_2_questions_2_answers)
{
    /* Resolved Ipv4 address */
    uint8_t data[] = {1,2,3,4};
    struct sockaddr_in key_addr;
    ...
    memcpy(&key_addr.sin_addr, data, sizeof data);
...
Ensure(pubnub_dns_handler, decodes_well_another_spooky_response_1_question_2_answers)
{
    /* Resolved Ipv4 address */
...
Ensure(pubnub_dns_handler, handles_response_with_1_question_and_no_usable_answer)
{
    ...
    /* Message shorter than first label stretch it contains?! */
    attest(pubnub_pick_resolved_address(m_buf,
                                        sizeof encoded_piece21 - 2,
                                        &resolved_addr),
           equals(-1));
...
Ensure(pubnub_dns_handler, handles_response_with_0_answers)
{
    struct sockaddr_in key_addr;
    struct sockaddr_in resolved_addr;
...
Ensure(pubnub_dns_handler, handles_response_with_an_issue)
{
    struct sockaddr_in resolved_addr;
...
Ensure(pubnub_dns_handler, handles_response_with_no_QR_flag_set)
{
    struct sockaddr_in resolved_addr;
...
Ensure(pubnub_dns_handler, makes_valid_DNS_query_request)
{
    /* Server name */
    ...
    append_request_question(name_encoded,
                            ...
                            RecordTypeA,
                            QclassInternet);
    attest(pubnub_prepare_dns_request(buf, sizeof buf, (unsigned char*)name, &to_send), equals(0));
...
Ensure(pubnub_dns_handler, handles_too_small_query_request_buffer)
{
    /* Server name */
    ...
    append_request_question(name_encoded,
                            ...
                            QclassInternet);
    /* Shorter buffer */
    attest(pubnub_prepare_dns_request(buf,
                                      sizeof buf - 1,
                                      (unsigned char*)name,
                                      &to_send),
           equals(-1));
...
Ensure(pubnub_dns_handler, handles_name_with_label_stretch_too_long)
{
    /* Server name */
    char const name[] = "to.a.CrambambulipanchiLabelStretch_tooLongAnd_Just_a_little_bitLonger.domain";
    uint8_t buf[100];
    int to_send;
    attest(pubnub_prepare_dns_request(buf, sizeof buf, (unsigned char*)name, &to_send), equals(-1));
    PUBNUB_LOG_TRACE("to_send = %d\n", to_send);
}

Ensure(pubnub_dns_handler, handles_name_with_label_stretch_with_no_length)
{
    /* Server name */
...
Ensure(pubnub_dns_handler, fires_asserts_on_illegal_parameters)
{
    int to_send;
    struct sockaddr_in resolved_addr;
    expect_assert_in(pubnub_prepare_dns_request(NULL,
                                                10,
                                                (unsigned char *)"pubsub.pubnub.com",
                                                &to_send),
                     "pubnub_dns_handler.c");
